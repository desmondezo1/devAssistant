...
exports.ConversationDto = exports.Roles = void 0;
var Roles;
(function (Roles) {
    Roles["user"] = "user";
    Roles["system"] = "system";
...
const conversation_dto_1 = __webpack_require__(2);
...
function activate(context) {
    ...
    let understandThisCommand = vscode.commands.registerCommand('devassistant.understandThis', async () => {
        ...
        if (editor) {
            ...
            const query = await updateQuery(selectedText);
            console.log({ query });
    ...
    const updateQuery = async (newQuestion = '', role = conversation_dto_1.Roles.user) => {
        const query = await context.workspaceState.get('conversation');
        const startingConversation = [
            { role: conversation_dto_1.Roles.system, content: 'you are a helpful assistant senior software engineer peer programming and assisting other engineers, ensuring they understand the code that they write and re-write it more efficiently' },
        ];
        if (!query) {
            await context.workspaceState.update('conversation', [...startingConversation, { content: newQuestion, role: conversation_dto_1.Roles.user }]);
            return [...startingConversation, { content: `Help me understand this code : ${newQuestion}`, role: conversation_dto_1.Roles.user }];
        }
        else {
            //remember to check if token size is less than max token.
            const oldConversation = context.workspaceState.get('conversation');
            if (role === conversation_dto_1.Roles.assistant) {
                await context.workspaceState.update('conversation', [...oldConversation, { content: newQuestion, role: conversation_dto_1.Roles.assistant }]);
                return [...oldConversation, { content: newQuestion, role: conversation_dto_1.Roles.assistant }];
            }
            else if (role === conversation_dto_1.Roles.user) {
                await context.workspaceState.update('conversation', [...oldConversation, { content: `Help me understand this code : ${newQuestion}`, role: conversation_dto_1.Roles.user }]);
                return [...oldConversation, { content: `Help me understand this code : ${newQuestion}`, role: conversation_dto_1.Roles.user }];
            }
    ...
    const getData = async (query) => {
        try {
            let data;
            const res = await axios_1.default.post('http://localhost:3000/chat', { query });
            data = await res.data;
            console.log({ data });
            const updated = await updateQuery(data.content, conversation_dto_1.Roles.assistant);
            console.log({ updated });
            return data;
        }
        catch (error) {
    ...
    const getHtml = async ({ selectedText, data = '', cssUriArray }) => {
